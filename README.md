...
### SoundMap — real, multi-device audio, visualized
A drag-and-drop 2D layout of your home built from your *real* devices — no placeholder rooms. Pair a Bluetooth speaker or name a device by hand and it shows up as a glowing, pulsing bubble you can reposition, rename, mute, or adjust individually — plus a master volume slider that scales every zone proportionally. Sign in and every device you're signed into stays on the SoundMap and plays the same track at the same time; volume, mute, rename, and position can all be controlled from any signed-in session, not just the device itself.
...
### MultiListen — real friends, real sync
Add friends by their account email, accept or decline incoming requests, and see who's actually listening right now. Tap **Listen Along** on a friend who's live and your session mirrors theirs in real time (read-only — nothing is written back to their account). The **Vibe Queue** below it demonstrates an algorithmic merge of two listeners' tastes, interleaving tracks by an affinity score.
...
**Bluetooth note:** pairing uses the real [Web Bluetooth API](https://developer.chrome.com/docs/capabilities/bluetooth) (Chrome/Edge on desktop or Android only — not Safari or Firefox) and confirms a real nearby device by name. Browsers have no way to route audio output to an arbitrary paired device, though — that's controlled by your OS's system audio settings, not JavaScript. Paired devices show up in the SoundMap for volume/zone management the same way any other device you add does.

**Friends & Listen Along:** once signed in, add another account by the email they signed in with (SoundMap's neighbor panel, **MultiListen → Friends**). They get a request to accept; once accepted, you can see when they're really playing something and tap **Listen Along** to mirror their session live and read-only.

**Security rules for production:** test mode leaves Firestore wide open. Before sharing this publicly, lock it down so each account can only touch its own devices/session, and friend data can only be written by the two accounts it involves, e.g.:
```
rules_version = '2';
service cloud.firestore {
  match /databases/{database}/documents {
    match /users/{userId} {
      allow read: if request.auth != null; // needed so friends can be found by email
      ...
      match /session/{docId} {
        allow write: if request.auth != null && request.auth.uid == userId;
        // Friends need read access to mirror a live session for Listen Along.
      ...
      match /friends/{friendId} {
        ...
        // Written on both sides when a request is accepted or a friend is removed.
      ...
      match /sentRequests/{toId} {
        allow read, write: if request.auth != null &&
          (request.auth.uid == userId || request.auth.uid == toId);
      }
    }
  }
}
...
└── src/
    ...
    │   ├── PlaybackContext.jsx    # playback, real YouTube audio, synced real devices, listen-along
    │   └── FriendsContext.jsx     # Firestore-backed friend requests/list, per-friend live session
    ├── components/
    │   ├── Sidebar.jsx
    │   ├── SearchBar.jsx
    │   ├── AccountPanel.jsx       # sign-in / sign-out UI
    │   ├── AddDeviceMenu.jsx      # Bluetooth pairing + manual device add
    │   ├── DeviceNamePrompt.jsx   # names + registers the real device you're on
    │   ├── SoundMap.jsx           # drag-and-drop canvas of your real, named devices
    │   ├── MusicMedia.jsx         # waveform, reactions, Liner Notes 2.0
    │   ├── MultiListen.jsx        # real friends, requests, listen-along + Vibe Queue
    │   └── PlayerBar.jsx          # persistent bottom transport bar
    ├── data/
    │   └── mockTracks.js          # local fallback catalog + reactions
    └── services/
        ├── youtube.js             # YouTube Data API v3 search client + fallback
        ├── youtubePlayer.js       # YouTube IFrame Player API loader (real audio)
        ├── deviceId.js            # stable per-browser device id + friendly label guess
...
- Firestore security rules restricting device/session/friend writes to the right accounts (this prototype assumes test-mode rules)
- `videos.list(part=contentDetails)` follow-up call for accurate YouTube track durations
- Friend search by username in addition to email
