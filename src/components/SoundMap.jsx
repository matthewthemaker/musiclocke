...
import { Volume2, VolumeX, Radio, Sliders, Trash2, Bluetooth } from "lucide-react";
...
function DeviceBubble({ zone, canvasRef }) {
  const { setZoneVolume, toggleZoneMute, moveZone, removeDevice, effectiveZoneVolume, isPlaying } =
    usePlayback();
    ...
    >
      <div className="relative flex flex-col items-center">
        ...
        <button
          ...
          className={`relative flex h-16 w-16 flex-col items-center justify-center rounded-full border border-void-line bg-void-panel ${accent.ring} transition-transform hover:scale-105`}
        ...
        <span className="mt-2 rounded-full bg-void-raised/80 px-2.5 py-0.5 text-xs font-medium text-ink-mid backdrop-blur">
          {zone.label}
        </span>
        ...
        {expanded && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="absolute top-20 z-40 w-44 rounded-xl2 border border-void-line bg-void-panel/95 p-3 shadow-glow-soft backdrop-blur"
          >
            <div className="mb-2 flex items-center justify-between gap-1">
              <span className="truncate text-xs font-semibold text-ink-hi">{zone.label}</span>
    ...
    <section className="flex h-full flex-col rounded-xl2 border border-void-line bg-panel-gradient p-5 shadow-inset-line">
      <header className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-ink-mid">Drag devices to regroup zones. Tap a bubble for local volume.</p>
        </div>
