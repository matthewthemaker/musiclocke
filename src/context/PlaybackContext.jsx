...
import { getThisDeviceId } from "../services/deviceId.js";
...
export function PlaybackProvider({ children }) {
  ...
  const pushSession = useCallback(
    ...
    [devicesSynced, user, track, isPlaying, progress, thisDeviceId]
  );

  useEffect(() => {
    if (!devicesSynced || !user) return undefined;
    const sessionRef = doc(db, "users", user.uid, "session", "live");
    ...
    return unsubscribe;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [devicesSynced, user, thisDeviceId, isYouTubeTrack]);
